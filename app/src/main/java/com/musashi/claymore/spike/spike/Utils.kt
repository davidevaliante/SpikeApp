...
fun SlotCard.getImageLinkFromName(size:ImgSize=ImgSize.MEDIUM):String{
    ...
    return  when(size){
        ImgSize.SMALL -> "$baseUrl${StorageFolders.Slot}thumb_${small}_slot_${this.name?.toSnakeCase()}$urlEnd"
        ImgSize.MEDIUM -> "$baseUrl${StorageFolders.Slot}thumb_${medium}_slot_${this.name?.toSnakeCase()}$urlEnd"
        ImgSize.BIG -> "$baseUrl${StorageFolders.Slot}${this.name?.toSnakeCase()}$urlEnd"
    }
}

fun Bonus.getImageLinkFromName(size:ImgSize=ImgSize.MEDIUM):String{
    val baseUrl = "https://firebasestorage.googleapis.com/v0/b/spike-2481d.appspot.com/o/"
    val urlEnd = "?alt=media"
    val small="64"
    val medium="250"
    return when(size){
        ImgSize.SMALL -> "$baseUrl${StorageFolders.Bonus}thumb_${small}_${this.name?.toSnakeCase()}$urlEnd"
        ImgSize.MEDIUM -> "$baseUrl${StorageFolders.Bonus}thumb_${medium}_${this.name?.toSnakeCase()}$urlEnd"
        ImgSize.BIG -> "$baseUrl${StorageFolders.Bonus}${this.name?.toSnakeCase()}$urlEnd"
    }
}
